...
/*** DOCUMENTATION
        <application name="Softmodem" language="en_US">
                ...
                <syntax />
                <description>
                        <para>Softmodem(hostname,port,options):  Simulates a
 FSK(V.23) or V.22bis modem. The modem on the other end is connected to the
 specified server using a simple TCP connection (like Telnet).</para>
                </description>
        </application>
 ***/
...
AST_APP_OPTIONS(
    additional_options,
    BEGIN_OPTIONS AST_APP_OPTION_ARG('r', OPT_RX_CUTOFF, OPT_ARG_RX_CUTOFF),
    AST_APP_OPTION_ARG('t', OPT_TX_POWER, OPT_ARG_TX_POWER),
    AST_APP_OPTION_ARG('v', OPT_MODEM_VERSION, OPT_ARG_MODEM_VERSION),
    AST_APP_OPTION('l', OPT_LSB_FIRST), AST_APP_OPTION('m', OPT_MSB_FIRST),
    AST_APP_OPTION_ARG('d', OPT_DATABITS, OPT_ARG_DATABITS),
    AST_APP_OPTION_ARG('s', OPT_STOPBITS, OPT_ARG_STOPBITS),
    AST_APP_OPTION('e', OPT_EVEN_PARITY), AST_APP_OPTION('o', OPT_ODD_PARITY),
    AST_APP_OPTION('u', OPT_ULM_HEADER), AST_APP_OPTION('n', OPT_NULL),
    END_OPTIONS);
...
typedef struct {
  ...
  int nulsent;    // we sent a NULL as very first character (at least the DBT03
                  // expects this)
} connection_state;
typedef struct {
  int sock;
    ...
    if (bit == 1 || bit == 0) {
      ...
      while (rx->fill >= (1 + databits + paritybits + stopbits)) {
        if (rx->bitbuffer[rx->readpos] == 0) { // check for startbit
          stop =
              (rx->readpos + 1 + paritybits + databits) % MODEM_BITBUFFER_SIZE;
          stop2 =
              (rx->readpos + 2 + paritybits + databits) % MODEM_BITBUFFER_SIZE;
          if ((rx->bitbuffer[stop] == 1) &&
              (stopbits == 1 ||
               (stopbits == 2 &&
                rx->bitbuffer[stop2] ==
                    1))) { // check for stopbit -> valid framing
            ...
            for (i = 0; i < databits; i++) { // generate byte
              if (rx->session->lsb) {        // lsb first
                ...
                if (rx->bitbuffer[(rx->readpos + databits - i) %
                                  MODEM_BITBUFFER_SIZE])
                  byte |= (1 << i);
              }
            }
            ...
            if (!paritybits ||
                (paritybits &&
                 (rx->bitbuffer[(rx->readpos + databits + 1) %
                                MODEM_BITBUFFER_SIZE] ==
            ...
            rx->fill -= 10;
          } else { // no valid framing (no stopbit), remove first bit and maybe
                   // try again
            rx->fill--;
            rx->readpos++;
...
static int modem_get_bit(void *user_data) {
  ...
  if (tx->writepos == tx->readpos) {
    if (tx->state->nulsent >
        0) { // connection is established, look for data on socket
      ...
      if (rc > 0) {
        ...
        for (i = 0; i < (databits + paritybits + stopbits); i++) {
          if (paritybits && (i == databits)) {
            tx->bitbuffer[tx->writepos] =
                (tx->session->paritytype == 2) ^ __builtin_parity(byte);
          } else if (i >= databits)
      ...
      if (tx->state->answertone > 0) {
        // 				ast_log(LOG_WARNING,"Got TE's tone, will send
        // null-byte.\n");

        if (tx->session->sendnull) { // send null byte
          for (i = 0; i < (databits + paritybits + stopbits); i++) {
            if (paritybits && (i == databits))
              tx->bitbuffer[tx->writepos] = (tx->session->paritytype == 2) ^
                                            __builtin_parity(0); // yes I know!
            else if (i >= databits)
              tx->bitbuffer[tx->writepos] = 1; // stopbits
            else
        ...
        if (tx->session->ulmheader) {
          ...
          int headerlength = sprintf(
              header, "Version: 1\r\nTXspeed: %.2f\r\nRXspeed: %.2f\r\n\r\n",
              tx_baud / (1 + databits + stopbits),
              rx_baud / (1 + databits + stopbits));
...
static int fsk_generator_generate(struct ast_channel *chan, void *data, int len,
                                  int samples) {
...
static int v22_generator_generate(struct ast_channel *chan, void *data, int len,
                                  int samples) {
...
static int softmodem_communicate(modem_session *s) {
  ...
  if (s->version == VERSION_V21) {
    modem_tx = fsk_tx_init(NULL, &preset_fsk_specs[FSK_V21CH2], modem_get_bit,
                           &txdata);
    modem_rx = fsk_rx_init(NULL, &preset_fsk_specs[FSK_V21CH1], TRUE,
                           modem_put_bit, &rxdata);
  } else if (s->version == VERSION_V23) {
    modem_tx = fsk_tx_init(NULL, &preset_fsk_specs[FSK_V23CH1], modem_get_bit,
                           &txdata);
    modem_rx = fsk_rx_init(NULL, &preset_fsk_specs[FSK_V23CH2], TRUE,
                           modem_put_bit, &rxdata);
  } else if (s->version == VERSION_BELL103) {
    modem_tx = fsk_tx_init(NULL, &preset_fsk_specs[FSK_BELL103CH1],
                           modem_get_bit, &txdata);
    modem_rx = fsk_rx_init(NULL, &preset_fsk_specs[FSK_BELL103CH2], TRUE,
                           modem_put_bit, &rxdata);
  } else if (s->version == VERSION_V22) {
    v22_modem = v22bis_init(NULL, 1200, 0, FALSE, modem_get_bit, &txdata,
                            modem_put_bit, &rxdata);
  } else if (s->version == VERSION_V22BIS) {
    v22_modem = v22bis_init(NULL, 2400, 0, FALSE, modem_get_bit, &txdata,
                            modem_put_bit, &rxdata);
  } else {
  ...
  if (s->version == VERSION_V21 || s->version == VERSION_V23 ||
      s->version == VERSION_BELL103) {
  ...
  while (!s->finished) {
    ...
    /* Check the frame type. Format also must be checked because there is a
       chance that a frame in old format was already queued before we set chanel
       format to slinear so it will still be received by ast_read */
    if (inf->frametype == AST_FRAME_VOICE &&
        inf->subclass.format == ast_format_slin) {
      if (s->version == VERSION_V21 || s->version == VERSION_V23 ||
          s->version == VERSION_BELL103) {
        if (fsk_rx(modem_rx, inf->data.ptr, inf->samples) < 0) {
          /* I know fax_rx never returns errors. The check here is for good
           * style only */
...
static int softmodem_exec(struct ast_channel *chan, const char *data) {
  ...
  AST_DECLARE_APP_ARGS(args, AST_APP_ARG(host); AST_APP_ARG(port);
                       AST_APP_ARG(options););
  ...
  if (args.options) {
    ast_app_parse_options(additional_options, &options, option_args,
                          args.options);
...
static int unload_module(void) { return ast_unregister_application(app); }
